...
void thread(void* args){
    ...
    if (pthread_mutex_trylock(&mutex)==0){
        printf("Thread %lu is painting!\n", pthread_self());
        //sleep(2);
        pthread_mutex_unlock(&mutex);
    } else {
        printf("Thread %lu is dont painting\n", pthread_self());
    }
}
...
void* paint(void* args){
    	...
    	// Our artist will now attempt to paint 5000 strokes of paint on our shared canvas
	for(int i =0; i < 5000; ++i){
		...
		if (canvas[painter->x][painter->y].r == 255 || canvas[painter->x][painter->y].r == painter->r &&
		canvas[painter->x][painter->y].g == 255 || canvas[painter->x][painter->y].g == painter->g &&
		canvas[painter->x][painter->y].b == 255 || canvas[painter->x][painter->y].b == painter->b) {
			if (pthread_mutex_trylock(&canvas[painter->x][painter->y].lock) == 0){
...
int main(){
    ...
    pthread_t moreArtists_tid[rookieArtists]; //making array of 50 artists
    artist_t* moreArtists = malloc(sizeof(artist_t)*rookieArtists); //mallocing 50 artists
   	
	// using for loop to create 50 artists pthreads
