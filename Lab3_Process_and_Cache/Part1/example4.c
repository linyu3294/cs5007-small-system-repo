...
int main(){
    ...
    if (fork() ==0){
        ...
        // If we want to do something the child_status here is an example.
        // Typically we can wait (NULL), but within our parent we may want
        // to capture how the child exited (maybe it exited with some error
        // code).
        ...
        // If the parent ends first(i.e. forgetting to call wait),
        // the child process is 'orphaned' and becomes a zombie
        // process that cannot be otherwise terminated.
